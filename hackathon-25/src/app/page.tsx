...
export default function ChallengePage() {
  return (
    <div className="min-h-screen p-4 md:p-8">
      ...
      <h1 className="text-3xl font-bold text-center mb-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto">
        {categories.map((category, index) => (
          <Link href={`/problem/${encodeURIComponent(category.name)}`} key={index} className="h-full">
            <Button
              variant="outline"
              className="w-full h-full min-h-[200px] p-8 flex flex-col items-center justify-center gap-4 text-lg font-medium transition-all hover:scale-[1.02] hover:bg-slate-100 dark:hover:bg-slate-800 dark:border-slate-700"
            >
